...
def fnDataImport(bDropNA=True):
    ...
    df = pd.read_excel("data/MarketData_no.xlsx", parse_dates=['Quarter'], index_col='Quarter')
...
def Main():
    ...
    for listModel in listModels:
        mSimulatedDistributions, mSimulatedPrices, listModel = cMS.fnPlotHistograms(
            listModel=listModel, listAsset=listAsset, sName='-'.join(listModel), Winsorize=False, sSave=True)
